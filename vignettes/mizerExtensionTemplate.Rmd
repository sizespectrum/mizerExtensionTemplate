...
(Sprat, Herring, Cod) model built with `newExtensionTemplateParams()`. It is
stored in `data/example_params.rda` and lazy-loaded by R, but the `.onLoad`
hook replaces the plain binding with an active binding so that every access
returns an object with the correct S4 extension class:
...
- [ ] For every `MizerParams` or `MizerSim` object bundled in `data/`, add a
  `makeActiveBinding` call in `.onLoad` (see `mizerExtensionTemplate-package.R`).
